...
namespace Rock.Rubick.SpellStillManager.Modes
{
    internal class StealModeBase
    {
        public virtual void ExecuteLogic()
        {
            ...
            lastSpell = Dictionaries.LastSpell.Where(x => x.Key.IsEnemy(General.localHero) && x.Key.IsVisible && !x.Key.IsLinkensProtected() && x.Value != main.Id && x.Key.Distance2D(General.localHero) < range && (x.Key.GetAbilityById(x.Value).Level + 1 >= main.Level
